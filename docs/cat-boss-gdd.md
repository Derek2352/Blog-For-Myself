...
**Version** 2.7 · **the cat looks up.** §17 is the first thing in this document that answers the reader before the reader has done anything — no reward attached, nothing to win, and the whole of it a reader of state the cat already kept. Before it: **the card is the only game — the fight leaves the page, and the page is left alone.** Five versions of
...
| 2.5.1 | **The dead zone gets a face, and the tool built to check it had the bug it was checking for.** 2.5 shipped §16 with one known hole recorded in §16.7: *nothing on screen distinguishes a flick that was too parallel from one that missed the cat entirely.* Both did nothing, for completely different reasons, and a player cannot learn which half of the gesture to fix from an outcome that never varies — the graze is the informative one, because it means the aim was right and only the angle was wrong. `contactFor()` now classifies a crossing swipe as `shove` or `graze`, and the graze shows a tell: **the tail swept forward over the back** for `GRAZE_MS`, with no accent, no body travel and **no cue** — feedback about aim is something a learning player triggers constantly, and a sound on a mistake you are about to repeat is how a mechanic becomes irritating. `cat-sfx.ts` keeps one cue per visual event; it does not owe every visual event a cue.<br><br>**Three things fell out of doing it properly.** (1) **The type had a silent case of its own.** `contactFor` first returned `'shove' \| 'graze' \| 'none'`, on the reasoning that a zero-magnitude rejection is not really contact — and the property test that walks every angle found `veer` returning *exactly* zero at 0° and 180°, where the arithmetic is axis-aligned and the floating point cancels. So the one function written to remove a silent outcome had one at precisely the two angles a player aiming down the cat's back produces. The caller was accidentally safe, branching on `!== 'shove'`, which is the worst kind of safe: **the next caller to read the type literally reintroduces the bug.** A zero rejection *is* a graze, and the case is gone. (2) **The pose was chosen by rendering seven candidates at 28px, and the best-looking one was forbidden.** Rotating the tail negative raises it into a near-vertical stroke that reads beautifully — and is exactly the kitten's signature, which §0 and 2.3 made the one shape the boss never makes because it is what tells your side from its side at 16px. The flick sweeps *forward* instead, clear of every standing posture and clear of the squad. The ear was dropped from the tell in the same sheet: pinned on its own it is indistinguishable from neutral at true size, and the ears already carry three other meanings. (3) **Reduced motion needed its own answer rather than the swat's.** The swat's accent colour is ungated and its shake is garnish, so it degrades to a colour change and stays legible; a graze's whole tell *is* movement, so gating it would put the dead zone straight back to invisible for exactly the visitors who cannot see a twitch. The class holds the posture unconditionally and the animation only makes its arrival a flick rather than a snap — §11's motion row is a promise about vestibular safety, not a licence to drop information.<br><br>**And the comparison rig had the fault it was built to catch.** `graze-look.mjs` renders the three outcomes side by side, and its first version carried a comment saying it froze the cat so the frames would differ only by class. It froze nothing: it relied on `locator.screenshot()` cropping to the element, but Playwright measures the box and *then* captures, and at ~300ms a capture the cat walks about its own body width — producing three frames each cropped to where the cat had been, showing legs and a tail sliding out of frame. **A comment asserting behaviour that no code performs**, committed inside the tool built to find exactly that, and caught only by looking at the picture. It pins the sprite with an `!important` stylesheet rule now, which beats the per-frame `style.transform` without reaching into the closure. **Then the browser found something no unit test could: the dead zone was unenterable.** With the tell in place, `hand.mjs` §1 was rewritten to *sweep* a half-circle of flicks at 12° spacing — a spacing that guarantees one lands within 6° of the estimated heading — and to record, at each contact, the angle the card itself saw. It produced **14–15 shoves and 0 grazes out of 15**, twice over. Probing the geometry showed why, and it is structural: the cat's heading is the line to a *walking kitten*, so it rotates **6–11° during the ~104ms a flick takes** (36° when the cat is on top of its quarry), and the harness's own reconstruction of it is only good to ±10°. **A dead zone narrower than its own axis's drift is one nobody can enter**, and a human finger has exactly the same problem. `SHOVE_MIN` is now stated as `SHOVE_MIN_DEG = 20°` with the threshold derived from it, because the angle is the quantity the design reasons about and the magnitude is downstream — and the widening is affordable *only* because the graze has a tell, which is a pleasing dependency: the feedback buys the rule, and the rule makes the feedback worth having. §16.7 records what remains open, which is that even at 20° a deliberate graze takes on the order of fifteen attempts, and names the most promising fix — **define the heading as the cat's own velocity rather than the direction to its quarry**, since that is a line the player can actually see it travelling along, and is what `trySwipe`'s comment already claims.<br><br>**The check that found it was wrong four times first, and every wrong version failed on the fixture rather than on the build.** It asked for a contact that was simultaneously near-parallel (to exercise the zone) and far from the quarry (so the angle is measurable) — two conditions that are nearly mutually exclusive in a live fight, since 9 of 14 contacts in one run happened within 40px of the kitten. Split in the end into two claims needing different evidence: the shove is an **assertion**, and the graze is a **roll** — re-flicked until it lands, reporting which attempt it took. That attempt count is not bookkeeping, it *is* the measurement, and a flat pass/fail could never have reported it. §12.1's split, applied at the level of a single flick.<br><br>**Gate:** 531 unit tests, `astro check` clean, warning-free build, `hand.mjs` 9/9 three runs running (its §1 now asserts the lengthways flick *grazes* rather than merely shoving less often, and §4 checks the shipped modes produce neither tell), `graze-look.mjs` 2/2, and the card and arena harnesses re-run green. |
| 2.5.2 | **Six fixes: three comments that lied, and three faults §16.7 had admitted to — and the one that mattered was that the axis was invisible.** 2.5 and 2.5.1 both ended by writing down what was still wrong rather than quietly leaving it, and that list was the work.<br><br>**The change that matters: the cat's heading is its own velocity.** `trySwipe` took it as `quarry − boss` — the line to whichever kitten the cat was chasing, **drawn nowhere on the board** — so a player asked to swipe "across its path" was being asked to aim relative to something they could not see. The consequence had been measured twice and misdiagnosed twice: `SHOVE_MIN`'s dead zone was found unreachable at 8.6°, widened to 20°, and found *still* barely reachable, with a swept half-circle producing 14–15 shoves and 0 grazes. **The width was never the problem.** The heading is now smoothed over `HEADING_TAU_MS` (220ms — roughly what an eye integrates a direction over, and conveniently the smoothed version of the line it replaced, which is what damps the 6–11° drift a flick suffers). Measured effect, which is how the fix was judged rather than argued: **a deliberate graze went from ~15 flicks to 1**, on the first flick of the sweep, on every run. The lesson generalises past cats — *if a mechanic asks the player to judge a quantity, the quantity has to be on screen.* The same heading now serves as `boss.shoveHx/Hy` too: "the line it was knocked off" and "the line you aimed across" are the same line, and were two different ones for no reason.<br><br>**The speed floor stopped being silent.** `isSwipe` returned without a word, so "you missed the cat" and "you crossed it too slowly" wore the same face — the identical two-failures-one-face fault 2.5.1 fixed for angle, one level down. `segmentHit` is now tested *before* speed, so only a segment that actually touched the animal can earn a tell, and a crossing swipe above `SWIPE_TRY_FRACTION` of the floor gets the **same graze tell** as a lengthways one. Deliberately the same picture: both mean *you touched me and got no purchase*, and which remedy to try is something a player works through anyway. Below the band a pointer is drifting, and a game that answers drifting is noise rather than feedback.<br><br>**And the verb survives being read.** §16 has no chip, so one caption at open was the only thing naming the gesture — and `arenaCaption` overwrites it on the first reclaim, which can happen before anyone has swiped once. It is restated every `HAND_HINT_EVERY_MS` until the player makes contact of either kind, then never again. The flag itself stays: three modes on a portfolio is still too much game.<br><br>**The three comments that lied**, all left by the two preceding commits: `trySwipe`'s doc claimed the heading was "its own velocity this frame, taken as the direction to its quarry" — two different lines described as one, and **Fix 1 is what made the first half true**; the dead zone was described as "about 9° wide" when 2.5.1 had widened it to 20°; and the graze was described as "flicks its tail and pins its ears" when the ears had been dropped after the pose sheet showed they read as nothing. A comment that lies about the code is the same fault class as state that never reaches the eye, which is most of §12.<br><br>**Two instrument bugs, and the first is the more embarrassing.** `WATCH_CONTACTS` computed its smoothing from the imported `HEADING_TAU_MS` — a **Node** binding, inside a function that runs in the **page** — so it threw `ReferenceError` on the first frame, the observer was never installed, and the sweep reported `0 contacts from 15 flicks`. That looks exactly like the game refusing to register a swipe; the give-away was that `window.__grazes`, set by a different page function that closes over nothing, kept counting perfectly. And §1's classification check **conflated the two graze causes** it had just created: a segment too slow to be a swipe grazes whatever its angle, so including those compared the card's answer to a question it was never asked — six "misclassified" grazes at sin 0.5–1.0 in one run, every one of them correct.<br><br>**Gate:** 543 unit tests, `astro check` clean, warning-free build, `hand.mjs` 9/9 three runs running, `graze-look.mjs` 2/2, `hand-look.mjs` 8/8 (the shove still lands: 14.0px peak against commander's 0.0), and the full twenty-harness fleet green on one build — arena 55/55, arena2 29/29, arena3 34/34, arena4 39/39, arena5 22/22, arena7 25/25, arena8 27/27, battle 25/25, card-ambient 5/5, card-check 21/21, card-fight 16/16, card-mechanics 11/11, check-ink 20/20, commander 15/15, cycle, first-run, top-state 18/18, touch-fight 31/31. **What §16.7 still says is open** is now the right question rather than the old one: whether 20° is the *right* width now that it is reachable — and that needs a person, not another harness. |
| 2.6 | **The hand becomes the game, and §15's orders retire with it.** §16 was built to be judged beside commander mode and never was, so it was confirmed rather than inferred: *the invisible hand is the default way a visitor intervenes while the kittens and the cat run themselves*, and **the swipe replaces click-to-order**. That is §16's founding argument — one job instead of two — promoted from a prototype behind `?cat=hand` to the shipped default. There are still exactly two modes, which is what the flag was protecting against in the first place: `hand` and §3's `manual`, with the chip naming the second.<br><br>**The default is renamed, not repurposed.** `'commander' | 'manual' | 'hand'` → `'hand' | 'manual'`. Keeping the name `commander` for a mode you can no longer command with would be precisely the lying-name fault 2.5.2 spent a commit removing from three comments — and it turned out to cost nothing, because **not one call site in the fleet passed `mode: 'commander'`**; they all relied on the default. `FightState.commanding` became `manual`, which is the bit §8's dialogue actually asks (*does the visitor hold claims themselves?*) rather than the inverse named after a retired verb, and an unspecified state now reads as the default game — which is the correct answer for a state nobody bothered to specify.<br><br>**What went, and the note left in each place.** `order()`, `assignOrder`, `Kitten.ordered`, and — by the same rule that governs the list — the `'order'` **cue**. `cat-sfx.ts`'s type carries the rule *"one per visual event, so this list grows only when something new is visible"*, and 2.6 is the first time it shrank: a cue for a verb nobody can perform is the exact inverse of the fault 2.5 fixed, which was a visible event with no cue. The `teach-order` ribbon line — *"point at one. they'll fetch it."* — became `teach-swipe`, *"swipe across me. i dare you."*, in the cat's own bluffing voice; leaving it would have spent the opening seconds instructing visitors to use a verb the game no longer answers, which is about the worst thing §8's table could contain.<br><br>**Two things the plan got wrong, both discovered by looking rather than reasoning.** It assumed `commander.mjs` would need its order checks retargeted — and **not one of its fifteen checks was about ordering**. What that file measures is *"open the fight, touch nothing, and watch the page come back"*, which a change to the player's verb does not touch at all; only the words moved, and the filename stayed so the GDD's citations of it still resolve. And the mode chip: 2.5.1 hid it whenever `swiping()` was true, because a two-state switch had nothing honest to say about a third mode. With `hand` as the default that hid the chip *always* — the compiler cannot see a boolean that changed meaning. 2.6 removed the third mode instead of the control, so the chip is back and always visible.<br><br>**And one thing I added a commit ago and deleted here.** 2.5.2 gave the caption a restatement of the verb, on the explicit premise that *"§16 has no chip, so the caption is the only thing that names the verb"*. The cat teaches it now, so the premise was false — and rendered, the ribbon said *"swipe across me. i dare you."* with the caption saying "SWIPE ACROSS THE CAT TO SHOVE IT OFF COURSE" thirty pixels below it in a different type style. The same fault as 2.2.1's `treats: 0 / 7 treats`: one thing drawn twice in two corners. `HAND_HINT_EVERY_MS` and its two tests went with it. **A justification that stops being true takes its code with it.**<br><br>**Retargeting twenty harnesses turned up three claims that were not true where they could be seen — 2.5.2's fault in three new disguises.** The fleet's one honest red was `hand-look`, which asked for `mode: 'commander'` and got the default: `fresh()` only branches on `'manual'`, so a mode that no longer exists silently becomes the one that does, and the strip would have presented two identical runs as a comparison. Its declared-mode assertion caught that exactly as designed. The fix was not to rename the control but to change what it holds constant: **the control moved from a mode to an axis.** Switching the hand off is no longer available without also changing what the cat is doing — manual mode makes it stalk the pointer, and `swipeCat` dispatches its flick as pointer events on the board, so the control strip would show a cat being dragged across the frame by the very gesture meant to do nothing, and a reader comparing two strips cannot subtract that. So: same mode, same machinery, same cat behaviour, and the only difference is the angle of the flick — which is the exact quantity `veer()` is about, since the shove is the perpendicular rejection of the swipe against the heading and a flick *along* the heading has nothing to reject. It also measures better than the mode it replaced. The old commander control walked **7.8px before the flick and 17.9px after** — the walk itself nearly doubling, muddying the one comparison the strip exists for — while the along control reads **9.1px → 9.1px, heading turned 0.0°, 1.9px of stray**, identical to a tenth of a pixel, against `across`'s **5.3px → 18.4px and 96 → 448px/s**. A control that holds everything constant except the thing being demonstrated beats one that switches off a subsystem and asks the reader to trust nothing else moved with it.<br><br>**The second was worse, because it was green.** `card-mechanics` asserted *"commander click orders a kitten (no ammo spent)"* by clicking a claimed tile and checking that *some* kitten had a `data-seek` — and it passed on the day ordering was deleted. It had to: `pickWork` reassigns every kitten every frame, so a squad on a board with claims on it always has somebody seeking something. The assertion never depended on the click, and would have passed with the click commented out. **A check whose subject is gone but whose evidence is supplied by unrelated machinery is worse than no check, because it reports a green.** What replaced it measures the rule that actually shipped — the tile under the pointer no longer changes what a click means, so a throw over a claim costs a treat exactly like a throw over bare board (`3 → 2 in hand`) — and unlike the old one it cannot pass without the click, because ammo only moves when a treat leaves the HUD.<br><br>**The third was a mechanic with unit coverage and no gate.** `teach-swipe` is the first thing a first-time visitor reads, and `first-run.mjs` — the harness whose entire job is the cold start — did not look at it, because until 2.6 the default mode had no verb to teach. It now opens the default, asserts a line arrives inside the opening grace, and asserts the verb it names is the swipe. **A default that acquires a verb acquires a new way to strand somebody who has never seen the game**, and the harness that owns first impressions has to be told when that happens.<br><br>Alongside the game: **the cover placeholders stopped announcing an unfinished site** (the `COVER · PENDING` stamp gone, the plate keyed to its category hue, `npm run covers` committed to keep the twenty-four in step with a generator that will change again), and **`arena7`'s last standing flake was fixed** by splitting a fixture from an assertion, counting offers instead of seconds, and watching a tile *leave* `unclaimed` rather than racing to catch it at `claimed` for one frame. **Gate:** 538 unit tests across 19 files, `astro check` at 0 errors / 0 warnings / 0 hints, a warning-free build, and all twenty harnesses green on one build — arena 55/55, arena2 29/29, arena3 34/34, arena4 39/39, arena5 22/22, arena7 26/26, arena8 27/27, battle 25/25, card-check 21/21, card-fight 16/16, card-mechanics 13/13, check-ink 20/20, commander 15/15, first-run 5/5, hand 8/8, hand-look 8/8, graze-look 2/2, top-state 18/18, touch-fight 31/31, plus card-ambient and cycle. |
| 2.6.1 | **The plates learn the dark, and the flag that would have told them was refused on purpose.** 2.6 left the covers glowing: the plate is a static SVG behind an `<img>`, so no page CSS reaches inside it, and the site's dark mode is class-driven — `.dark` on `<html>`, persisted to `localStorage` — so an in-SVG `prefers-color-scheme` query would answer to the system while the visitor is answering to the toggle. Twenty-four sand-coloured lightboxes in a dark room, documented last pass as a limitation rather than fixed.<br><br>**The plan's design was a `coverPlaceholder` flag in frontmatter, maintained by `npm run covers`. It was rejected after being written down, and the reason is the reason this whole run exists.** It would work. It would also be a second copy of an answer the file already gives — and a second copy is a claim that can go quietly false, which is the single fault 2.5.2, 2.6 and this row have now spent four commits removing. So the site reads the file instead: `src/lib/cover-plate.ts` pulls every `cover.svg` through `import.meta.glob` with `?raw` at build time and asks one question of the text. Drop a photograph in, delete the plate, redesign the drawing — the answer moves with the file, and there is nothing anyone has to remember to update.<br><br>**The question is one string, in one place, because it already failed as two.** `redraw-covers.mjs` shipped in 2.6 with a detector that tested for the caption the same commit was deleting: *a detector that only recognises the output it is replacing can run exactly once*, and it reported "redrew 0, left alone 24" as a success. The fingerprint now lives in `scripts/cover-plate.mjs` — dependency-free, so the generator and the Astro build can both import it without either dragging the other's runtime along, the `photo-rules.mjs` precedent — and `PLATE_MARK` is the `<pattern id="ledger">` that draws the grid, the one element every plate the generator has ever made contains. `tests/images.test.ts` holds the two to each other: the plate `placeholderSVG()` draws **today** must carry the mark, across three seeds × three hues including the omitted-hue `resolveWash()` fallback branch, so a redesign that drops it fails in the suite instead of silently un-recognising twenty-four covers. **`isPlateSVG()` is deliberately not `isPlaceholderCover()`**, and the two are allowed to disagree: one asks *did our generator draw this*, the other asks *does this entry still want a photograph* — a hand-drawn diagram answers yes to the second and no to the first, and collapsing them would either invert a diagram or leave a plate glowing.<br><br>**The filter is `invert(1) hue-rotate(180deg)`, and the second function is not decoration.** Inversion alone reverses lightness *and* hue: the sand plate at `hsl(12 24% 88%)` comes out a cold cyan, the one colour the site does not contain. At 180° the hue-rotate matrix is cos = −1, sin = 0, giving `R' = −0.574R + 1.430G + 0.144B` and its two siblings — enough to return the hue while leaving the reversed lightness alone. Measured on the built site rather than predicted: light plate **rgb(232 220 217)**, unchanged, matching the SVG's own 88%; dark plate **rgb(42 30 27)** against the dark surface token `#272119` = rgb(39 33 25). The plate lands on the card it sits in, within three points per channel, which is what "sinks into the ground" has to mean to be checkable.<br><br>**The entry hero lost its blur-up in the same edit, because on dark it was not a placeholder but a flash.** A blur-up previews a photograph arriving; a plate has nothing to preview — it is flat sand, a grid and a crosshair, and the build renders it sharp. Worse, `.blurup img` holds the image at `opacity: 0` until it loads, so the light LQIP painted first and the filtered plate arrived over it: a bright rectangle on every plate entry page, in a dark room, once per navigation. Both are gone for plates and untouched for photographs.<br><br>**And the harness that was built to check all of this had the session's fault too — the fourth instance, in the newest file in the repo.** `plate-look.mjs`'s first version opened `browser.newContext()` instead of the fleet's `fresh()`, so the first-visit `<dialog class="welcome">` opened over the page and its `::backdrop` dimmed every screenshot: the light plate came back **rgb(139 129 123)**, a grey-brown, against an SVG that plainly says 88% lightness. The number was wrong and looked like a finding. `document.elementsFromPoint` named the modal in one call. **A screenshot harness that photographs a modal it did not notice will report anything you like**, and the fix — as every time — was to look at what was actually rendered instead of trusting the figure it produced. It now shoots **both** code paths, since the grid reaches the plate through `EntryCard.astro` and the hero through `[...slug].astro` where the blur-up was removed, and asserts `blurup` is absent rather than assuming the edit took. One honest asymmetry it surfaced: the hero's dark plate reads rgb(50 34 37) against a card of rgb(39 33 25), so a plate that sat *below* its card in the light sits slightly *above* it in the dark. Reversing lightness reverses that relationship; nothing short of a second drawing preserves it, and a second drawing is a second copy. **Gate:** 541 unit tests across 19 files (three new, holding the generator to the detector), `astro check` at 0 errors / 0 warnings / 0 hints, a warning-free build, `npm run covers` reporting "redrew 24, left alone 0" and producing byte-identical files, and both themes of both code paths read off the composited page. **Corrected by 2.6.2:** the headline of this row overstates it. Twenty-four covers and the entry hero learned the dark; the **home page hero** did not, and it is the largest cover on the site and the first one anybody sees. `index.astro` built its own `blurup`/LQIP branch rather than going through `EntryCard.astro`, so the fix reached every copy but the one on the front page — the same second-copy fault this row congratulates itself for refusing, committed one file away from where it was refused. **And by 2.6.3:** *"The plate lands on the card it sits in, within three points per channel, which is what 'sinks into the ground' has to mean to be checkable"* is true of the plate it was measured on and of five others. Measured across all twenty-one, eleven were more than 8 points off and the worst was 23 — the check was right, and one sample of a twenty-one-member population was not a check of the population. |
| 2.6.2 | **The treats readout stops being furniture, the pages nobody had looked at get looked at, and the probe built to settle it was wrong three times first.** The paw row was made permanent so that a visitor's first sight of it would contain the word *treats* and the seven dots would stop being unexplained debris — a pre-GDD change (`1806e94`) documented on `tallyFor` and never written into this document, which is why two comments in `SiteCat.astro` had been crediting **§9.4** for it. §9.4 is the handicap ladder; the only thing it puts in this row is a withheld paw. That was the right problem and the wrong tenure. `#cat-hud` is fixed at `left: 1rem`, the chip measures **111px** at rest (`"0 / 7 treats"`), 172px on `"1 / 7 · watching you"` and **263px** on the longest phrase the game can produce, while `.wrap` (max-width 72rem, `padding-inline: 1.5rem`) starts the content column at **88px @1280 and 24px @1024** — so there is no gutter to sit in below about **1374px**, and an opaque chip that never leaves is an opaque chip parked on the writing of a site whose entire premise is the writing.<br><br> **The previous pass had already read this fault and mis-scoped it.** 2.6.1's working tree grew `--cat-hud-height` and `--cat-hud-clear`, and the footer padded itself so the chip would clear it; `SiteCat.astro`'s comment then said the chip *"was covering the footer"* — past tense, fault closed. `scratchpad/hud-overlap.mjs` samples `elementsFromPoint` on a grid across the chip's rect and reports every text element the browser finds behind it, which is the check that matters because it asks *is a reader's word behind this panel* without deciding in advance which elements to look for. It found `h2 "Education"` on /about/ and `p "A post-payment ledger…"` on /competitions/ at **scroll-top**, `li "Korean — basic"` and `h2 "Or wander by subject"` **mid-page**, at both widths. The footer was not the fault; it was the one instance of the fault that happened to get screenshotted. **Padding cannot move the middle of a page out from under a fixed element**, so the reserve was deleted and the readout moved out from under the page instead — which also hands every page on the site its 104px foot back.<br><br> **The mechanic.** *Purpose:* keep the tally legible without spending a reader's paragraph on it. *Experience goal:* a notification, not furniture — the seconds it covers a word are the seconds it is asking to be read. *Inputs:* page arrival (once per **session**, not per navigation — `#cat-hud` is `transition:persist` chrome, so a per-navigation greeting would be the same box interrupting the same reader on every page of the site), and every treat found. *Outputs:* `.speaking` for `HUD_DWELL_MS`, and `.announce` swapping the tally for the level phrase over the same window. *`[PH 3200]`:* long enough for a glance down, five words and a glance back — the caption is at most `"7 / 7 · all found — good hunting"` — and short enough that a visitor who ignores it is not reading around a box for long. **One dwell covers both greeting and find deliberately**, because two would be two numbers to keep in step for a difference nobody can perceive. *Edge cases:* no categories at all → `data-empty`, `display: none`, and no greeting to issue; phone with the card open → the existing `:has()` rule stands the row down, unchanged; a find inside an open dwell → `clearTimeout` extends the window rather than stacking timers. *Failure states, each with the guard named:* **a readout that never retires** — both timers re-read `scoreBox()` instead of closing over the node, because a persisted element is still replaceable and a callback holding a detached one would clear `.speaking` on a node nobody can see while the live chip stayed up forever; **a greeting on every click** — the `greeted` latch; **a first sight of the row that never says "treats"** — `.speaking` (visibility) was split from `.announce` (which caption), which were one concept only while the row was permanent, and welding them back together would make a visitor's first sight of the row the flavour phrase instead of the tally.<br><br> **Three wrong instruments, one after another, all the same mistake wearing different clothes: standing something convenient in for the condition actually wanted.** (1) The probe scrolled with the default behavior and slept 250ms — `global.css` sets `html { scroll-behavior: smooth }`, so on a long page the "bottom" rows sampled whatever was passing mid-animation, and the run reported *"at 1024 even the footer is covered"*. **That finding did not exist**, and acting on it would have sent the pass off to fix a bug that was not there. `behavior: 'instant'` plus a poll until `scrollY` stops moving. (2) "At rest" was then defined as `HUD_DWELL_MS + 600` elapsed since load — true only if nothing speaks in between, and on /competitions/ @1024 the cat reaches that page's treat about a second and a half in, `announce()` restarts the dwell, and the row logged as *at rest* was a find still being announced. (3) So wait for `.speaking` to come off instead — except `#cat-score` carries `transition: opacity 0.45s` under `prefers-reduced-motion: no-preference`, so the class leaves ~450ms before the chip does, the probe caught it mid-fade at opacity ≈ 1 on every page, and the run reported a **permanent readout that no reader ever sees**. The same 0.45s bit the other way on the forced-visible state: reading `getComputedStyle` in the same frame as `classList.add('speaking')` returns exactly `0`, so the probe declared the chip hidden in the one state it was being forced to show. Both ends now wait on the number the eye actually reads. **A fourth would-be finding was retracted before it reached code** — that the chip swallows clicks on the entry link underneath it — because `#cat-hud` has carried `pointer-events: none` since it was written.<br><br> **What the corrected probe says, stated as the trade rather than rounded off to a win.** At rest, **24 of 24** rows (4 pages × 2 widths × 3 scroll positions) report the chip absent. Speaking at the 263px worst case, **16 of 16** rows still land on prose — `h2 "Education"`, `a "May 2026 · Selected Delegate…"`, `li "2026-07 E-021 Bank of China (Hong Kong…"` at the top, and the footer's tagline at the foot of every page at both widths, which is the cost of dropping the reserve and is recorded next to the token rather than left for someone to rediscover. The overlap is not gone; it is bounded to the moment the chip is the thing worth reading. What was unacceptable was a box parked on a paragraph for the length of a visit — three seconds of it is a notification. The cat itself stays permanently visible and remains the standing invitation: it is a small transparent sprite that can walk over a line without erasing it, and this chip cannot.<br><br> **Alongside it, the first pass over the pages the game has never touched.** The **monthly-log section** shipped a kicker, a 3xl display heading and the sentence *"No monthly notes filed under this one yet"* on all eight category pages, none of which has ever had a log — a section whose only content is an apology for itself, and roughly five hundred pixels of nothing above the footer. It is hidden in the build and kept in dev behind the same `import.meta.env.DEV` split the entries branch and `PhotoPlan.astro` already use: **the visitor never sees the scaffolding and the author never loses it**. The entries section keeps *its* empty state, because entries are why the page exists and arriving at an empty index and being told why is useful — the rule is that a *secondary* section's absence is not worth a heading. **`· 0 logs`** came off the home browse cards and the category headers for the same reason, matching a guard `TabBar.astro` already had. And the **`/about/` portrait**, which was a near-black rectangle reading `PORTRAIT` over `REPLACE ME`: a note to the author displayed to the audience, and the highest-contrast shape on the page whose whole job is a first impression. `npm run covers` now draws it from `placeholderSVG` at 4:5 (the frame a real photograph will arrive in, so the slot does not change shape on the day it is replaced) at hue 32 (the hue `about.astro` already passes to `Base`), which means it also carries `PLATE_MARK` and takes the dark filter through the new `isPlateAsset()` — globbing `*.svg` and nothing else, so a `portrait.jpg` answers `false` before anyone remembers there was a flag to clear. **Inverting a photograph of a person is the worst version of this bug available on the site**, and that is the guard against it. The admission moved to the `alt` text, where a reader who needs it gets it and a visitor is not shown the author's TODO list.<br><br> **Gate:** 541 unit tests across 19 files, `astro check` at 0 errors / 0 warnings / 0 hints, a warning-free build, and the full twenty-one-harness fleet green on one build — arena, arena2, arena3, arena4, arena5, arena7, arena8, battle, card-ambient, card-check, card-fight, card-mechanics, check-ink, commander, cycle, first-run, graze-look, hand, hand-look, top-state, touch-fight. `arena7` was run five times and went red **twice** — and both reds are the same line, reported as `FIXTURE` rather than as a failing assertion: *"a claim regrows under the parked pointer — 0 of 3 cycles reached"*. That is 2.6's split doing its job. The harness can now say *the setup never produced the situation this check is about* instead of blaming the game for it, and two in five sits squarely on the one-in-four rate 2.5 measured on both sides of a `git stash`. **It is not being called fixed, and it is not being called a game bug**; what is fixed is that the report no longer confuses the two. **Still not playtested by a person.** |
| 2.6.3 | **The page audit's last three findings, and all three changed shape the moment they were measured.** 2.6.2 fixed the one finding that was exactly as reported. These are the ones that were not, and the pattern is worth naming before the list: **a fault noticed in a screenshot is a real fault at a wrong address.** The eye is excellent at spotting that something is off and hopeless at saying which of three margins, five seeds and a colour matrix did it — so each of these got an instrument before it got a fix, and each instrument moved the finding.<br><br> **#5 — "350–500px of dead space above the footer on short pages" was one page, then two.** `scratchpad/tail-gap.mjs` walks every page shape at both widths and reports the distance from the last painted element inside `main` to the top of the footer, with the rules that contribute to it. Every page on the site reports **96px**, and 96px is `mt-24` on the footer — deliberate breathing room, not dead space. Exactly two pages do not fill their own viewport at 1280: **`/search/`** at a 327px gap and **`/leisure-time/`** at 212px, and both are short for the same reason rather than for a layout one — they have nothing on them. `/search/` was a heading, a box and a void, which is a fair design for a search page whose corpus the visitor already knows and the wrong one for a stranger's first stop from a CV: it asked them to guess a word. It now carries the ten busiest tags as starting threads and a sentence of real numbers (`tagCounts()`, read at build time so it cannot go stale, and busiest-first because that is a *different question* from `/tags/`'s alphabetical index rather than a second copy of it) — and the block hides itself once anything is typed, keyed on the input's value rather than on Pagefind's results, because a query with no matches still means the visitor has said what they want. It also moves the only helpful text on that page out from behind `<noscript>`, where the help was visible exactly to the people whose search box worked. `/leisure-time/` is the one category with nothing filed under it, and it now offers the ones that do have things in them, with counts — because the one thing a visitor on that page has just learned is that a category on this site can be empty. Its `0 entries` line went too, by 2.6.2's own rule: a count of nothing is not a fact worth printing, least of all three lines above a heading that says *Nothing here yet.* in the site's own voice. Both pages now report the standard 96px.<br><br> **#7 — "the phone tab-bar clips mid-glyph" was half already handled, and the unhandled half was the other edge.** There *was* a fade on the right: a 2.4rem `::after` washing `--color-ground` over the strip's last inch. It reached full strength 21px in, so the last glyph of "AI & Creative Projects" came out about a third covered — smudged rather than dissolved, which is what the audit saw. The edge with **no fade at all** was the **left**, and the audit could never have found it: it shoots pages at rest, and at rest `scrollLeft` is 0. `mask-image` on the strip's own content replaces the wash, at both edges, driven by `data-more-left` / `data-more-right` off the same scroll listener, with the two stops registered through `@property` so they are actually animatable — an unregistered custom property in a `transition` is a line that does nothing, which would have been this pass's own fault class in a new place. **And one claim of mine went in and came back out.** I wrote that the opaque wash mismatched the header's translucent pane once content scrolled under it. Measured: the header is sticky at a negative offset and the strip's top is at **-41px by scroll 400**, so the tab bar is only ever on screen near scroll-top, where an opaque `--color-ground` matched exactly. Fading content rather than covering it is still the better mechanism; it is robustness, and it is not written up as a bug that was fixed.<br><br> **#6 — "the `experience` hue (44) inverts to olive rgb(31 27 16)" was right about the hue, and finding out why took three wrong instruments and overturned 2.6.1's headline check.** 2.6.1 verified the dark filter on **one** plate — `competitions`, landing within three points per channel of the dark surface — and called the covers done. `scratchpad/plate-sink.mjs` measures all twenty-one built plates: **eleven were more than 8 points off on some channel and the worst was 23.** The cause is two mechanisms multiplying. `placeholderSVG` varied the ground's lightness by seed, ±4 points around 88%, on walls that were reasoned against the *light* page ground (96%); the dark theme reaches the plate through `invert(1) hue-rotate(180deg)`, and against a 12% surface that same ±4 is enormous — at 92% the plate lands 15–16 points *darker* than the card it sits in, at 84% it lands 11–23 points *lighter*. The low wall is real and it is at about 87, not the 83 the comment claimed. That leaves 87–90, three points, for five steps — under the two-point minimum the generator's own comment sets for a step being perceptible at all. **An axis that has to become invisible to be correct is not an axis**, so it is deleted rather than squeezed; the two remaining axes vary the drawing (grid pitch, rule inset), which the same comment already argues is the stronger signal. With the lift gone the residual is pure hue, and it is exactly the shape #6 named: `hue-rotate` is a fixed linear matrix, not a perceptual operation, so the two ends of the warm band miss in opposite directions — the pinks (340, 350) land **+11 on blue** and read violet *above* their card, and **44 lands −9 and reads olive** *below* it, which is the plate the audit spotted. Saturation swept 24 → 18 → 14 → 10 with all twenty-one re-measured at each: the count over 8 goes **10 → 3 → 3 → 2** while the median goes **5 → 5 → 6 → 7**, because past 18 desaturating stops buying outliers and starts costing the middle — the dark surface is itself a warm colour, so a neutral plate has to travel to reach it. 18% is where the curve turns. **Final: median worst-channel Δ 5, max 10, three of twenty-one over 8** — against 9 / 23 / eleven before. A unit test now pins the ground to one lightness across seeds, so the lift cannot come back quietly.<br><br> **Three more instruments were wrong first, which by now is the reliable part of this process.** `plate-hues.mjs` sampled each plate's exact centre and came back with seven convincing mauves in the 206–216 range — `placeholderSVG` draws the crosshair at the plate's exact middle in `#8e2f45`, so the sample was of the crosshair. The tell was that it disagreed with 2.6.1's verified rgb(232 220 217), which is the only reason it was caught: **a number with nothing to disagree with is a number nobody checks.** Then the fix moved the sample to a third of the way in, which read *something* — and reported one plate 16 points darker than the rest, which a hue rotation cannot do. What varies at a fixed point between plates is which bit of the drawing is under it, so the reading became a median over the plate's interior. And the unit test written to lock all this in matched `hsl\(32 \d+% …\)`, which finds the **grid's 34%** before the ground's 88%, and failed for the right reason before it guarded anything — the ground is the only `fill=`, the grid and frame are `stroke=`.<br><br> **Gate:** 542 unit tests across 19 files, `astro check` at 0 errors / 0 warnings / 0 hints, a warning-free build, `npm run covers` redrawing 25 of 25, and the full twenty-one-harness fleet green — arena 55/55, arena2 29/29, arena3 34/34, arena4 39/39, arena5 22/22, arena7 26/26, arena8 27/27, battle 25/25, card-ambient 5/5, card-check 21/21, card-fight 16/16, card-mechanics 13/13, check-ink 20/20, commander 15/15, graze-look 2/2, hand 8/8, hand-look 8/8, top-state 18/18, touch-fight 31/31, plus cycle and first-run. **Two flakes seen and named rather than rounded away.** `arena7` was run five times and went red twice, both times on the same line and both times reported as `FIXTURE` — *"a claim regrows under the parked pointer — 0 of 3 cycles reached"* — which is 2.6's fixture/assertion split doing its job: the harness can now say *the setup never produced the situation* instead of blaming the game. And `card-fight` went red once in three runs on *"rounds escalate without being told to"*, in a harness nothing in this pass touches. Neither is called fixed. **Still not playtested by a person.** |
| 2.6.4 | **The pages the audit had never actually read, and the scrollspy that was pointing at the wrong month for three quarters of a page.** 2.6.3 cleared the ranked findings. This is the other half of the review: `monthly`, `colophon`, `entry`, `tags` and the tag page at desktop, which had been shot and never looked at.<br><br> **`JumpRail` was marking one item, at one scroll position, and then keeping it.** `scratchpad/rail-spy.mjs` parks `/monthly/` at real scroll positions and reads the rail against what is on screen: **nothing marked at 0, at 1200 or at 2400**, then `October 2025` at the very bottom — and October **stayed lit all the way back to the top**. Two causes, and the first is the one worth having. `document.getElementById(a.dataset.jump)` returns the **heading**, because `PeriodGroup` puts the id on the `<h2>` and points the `<section>` at it with `aria-labelledby` — correct markup, and not what the observer assumed. A heading is ~40px tall and `rootMargin: '-10% 0px -80% 0px'` leaves a band a tenth of the viewport high, so the target intersected only while it transited about 130px of scroll; a section is in that band the whole time you are reading it. And the callback only ever *set* a mark (`if (entry.isIntersecting)`), so nothing ever cleared one. The rail now takes `el.closest('section') ?? el` — which is also right on `/timeline/`, where the ids sit on 40px `<li>` year markers with no section anywhere — and computes the answer from rects: **the last target whose top has passed the reading line**, which is "the section I am in" on one page and "the last year I scrolled past" on the other, and is correct in the gaps where nothing intersects anything.<br><br> **Keeping the observer as a cheap trigger was wrong, and it took a second measurement to see.** The moments its entries flip looked like exactly the moments the answer changes. They are not, when the target is small: jump `/timeline/` 400px and the 2026 marker goes from *below* the band to *above* it in one frame, `isIntersecting` is `false` on both sides, no callback fires, and the rail marks nothing at all. The first fix left eleven-item `/monthly/` correct at every position and two-item `/timeline/` blank at every position — which is what a fix looks like when it is only tested where the bug was found. **A boolean that happens to be equal before and after is not a way to detect that something moved.** A passive `scroll` listener coalesced into one `requestAnimationFrame` is, and the work is a handful of `getBoundingClientRect` reads on a list that is never longer than the rail is tall. Both rails now track at every position and go blank above the first section, which is the honest answer when the reader is in the page's own header.<br><br> **`/tags/` opened by claiming something its own contents denied.** *"The same interests keep surfacing across categories. Pull a thread"* — above forty-eight identical chips, of which **thirty-five lead to a page with one entry on it**. A thread that runs through one thing is not a thread. The thirteen recurring tags now lead with their counts and the singles follow in quieter type under a label that says what they are. Nothing is hidden — an index is complete or it is not an index, and a one-entry tag page is a perfectly good destination from the entry that carries it — but the two have stopped being presented as the same kind of thing. Both halves stay alphabetical, unlike `/search/`'s starting threads which are busiest-first: **the orders answer different questions.** There you arrive with nothing and want to be handed a word; here you arrive with the word and want to find it.<br><br> **Two things I was sure of from the shots, both wrong.** The colophon's column *looked* long beside the entry pages'. Counted in `ch` from each paragraph's own computed font (`scratchpad/measure.mjs`): colophon **65**, entry **68**, about **65** — all inside the 60–75 band that is comfortable for continuous reading, and the impression came from the two pages using different type sizes. What the same run did find is that 2.6.3's own new sentence on `/search/` ran to **82 characters** a line, small type inheriting a container capped for the search *results*, which are a list; it is `max-w-prose` now and measures 65 like everything else. And the home hero's `max-w-[48ch]` looked like a cap that does nothing, since the paragraph measures 37ch at 1280. Measured across six widths it is inert at five and **load-bearing at 768**, where the hero is still one column and the line would otherwise run 64ch. Left alone.<br><br> **The measuring tool needed correcting twice more, which is now simply what this costs.** `rail-spy.mjs` first looked for `aria-current` or an accent colour and reported "nothing marked" everywhere — the detector, not the page. Its second version inferred the mark by finding the item unlike its neighbours, which cannot work on a **two**-item rail: with one marked there is no majority and with none there is no minority. It reads the `.here` class directly now, which was available the whole time in the file being debugged. And `measure.mjs` first flagged the home page and category pages as having dangerously short lines — it was measuring **card excerpts**, and a 160-character summary in a 352px grid cell is a card, not a column; it also flagged every 390px row, where 41ch is what a phone gives you and says nothing about the design.<br><br> **Gate:** 542 unit tests, `astro check` at 0 errors / 0 warnings / 0 hints, a warning-free build, and arena 55/55, arena4 39/39, check-ink 20/20 re-run after the rail change — the three harnesses that walk the page chrome the rail lives in. **And the fleet is twenty-two now**, because nothing in it asserted on `.jump-rail` and that is exactly why the fault survived eleven versions. `rail-spy.mjs` started as a probe and became a check: ten assertions across both rails — the rail exists, it says nothing above the first section, it marks the section under the reading line at seven positions spread over the page, it actually moves rather than latching, and it clears again on the way back up. **Verified against the bug it exists for**, because a check that cannot fail is worse than no check — `card-mechanics` spent a version green on an assertion whose subject had been deleted. With the pre-2.6.4 script pasted back in it reports **6/10**, and every one of the four failures is a symptom that was first seen by hand. `tests/harness-hygiene.test.ts` picks it up by directory scan and passes 7/7 on it unchanged.<br><br>**One more thing the empty category turned out to be doing.** `/leisure-time/` was in `sitemap-0.xml` with **no `robots` meta at all** — a page whose entire content is *"Nothing here yet."*, submitted to search engines as a result somebody might want, while being unreachable by clicking, because `getNavCategories()` already and correctly drops empty categories from the tab bar. It carries `noindex, follow` now — `follow` rather than `nofollow` because the page is thin, not untrusted, and 2.6.3 gave it a row of links to the categories that do have things in them. Read off the page's own contents rather than kept in a list of exceptions, so it lifts by itself the day something is filed there; verified in `dist`, where the tag is on `/leisure-time/` and on no other category page. The sitemap still lists the URL, and that is stated where the flag is set rather than glossed: `@astrojs/sitemap`'s filter sees a path string and has no access to content counts, so `noindex` is the authoritative signal. **Still not playtested by a person.** |
| 2.7 | **The cat looks where you point (§17), and the copy was measured rather than imitated.** Asked for from an 11-second recording of an unrelated landing page whose sunflower turns toward the cursor. What got copied is the *mechanic*, established with `scratchpad/measure_ref.py`: the reference's face travels **~17px horizontally against ~3px vertically**, a 5.7:1 ratio, and the dark region's area shrinks at the extremes — so the thing that reads as *it noticed me* is a **yaw**, not a nod, and that ratio is now a named constant. Getting the frames out took its own detour: this container's Chromium has no H.264 and Playwright's bundled ffmpeg turns out to be a three-demuxer VP8-only build, so a real one came in through npm.<br><br> **One claim is deliberately missing from that write-up.** The first attempt to correlate the effect with the cursor looked for "near-white, anywhere" and locked onto the page's white headline, reporting the same coordinate in all eleven frames — and would have produced a confident "mean direction agreement +0.03", a number measuring a static object. That the reference follows the pointer is established by reading frames. No figure is quoted for it.<br><br> **Two answers from Derek collapsed most of the work.** *The cat does the looking* rather than a new mascot, and *a touch is looked at*. Both together mean the feature adds **no listener, no element and no art** — it reads `prey`, the pointer record the cat has kept since 1.1, inside the rAF `tick` that already runs. "Look at where I touch" needed nothing built at all: the touch `pointerdown` path already logs a tap as `sight(x, y, 5200)`, one sighting held far longer *because a finger reports once and then there is no pointer any more*. It also dissolved the one constraint that looked fatal — `snapshotOf` records the `style` attribute of **every** element on the home page, so a per-frame style write would have reddened `arena` and `arena4` anywhere else on the page; `#site-cat` is already excluded.<br><br> **Three things about the drawing that had to be got right, none of which are visible in a screenshot.** (1) The independent `rotate:`/`translate:` properties, never `transform:` — `cat-blink` on the eye and `cat-groom`/`cat-munch` on the head all animate `transform`, so writing it here would fight three shipped animations and **the cat would stop blinking the moment it looked at you**. 2.5 learned the same thing one level up when the swat keyframes had to leave `transform` alone because it already carried the facing flip. (2) `.cat-gaze` wraps `.cat-head` rather than replacing it, because giving the head a neck origin would silently redraw two shipped poses. (3) **SVG's `transform-origin` resolves against the view box**, so an unset origin swings the head about (32, 20) — the middle of the sprite, below and left of the skull — and the glance arrives as a shoulder throw.<br><br> **The fault worth building a harness for is the mirror.** The flip is `scaleX(-1)` on the `.cat-flip` ancestor, so a rotation applied inside it turns the wrong way and the sign must be negated on `s.dir === -1`. Get it wrong and the cat looks at your cursor's *reflection* — which is invisible on a static screenshot and only shows up if you watch it walk back the other way. **And the first version of the check passed for the wrong reason**, reporting a 40px swing against 2.4px on the un-mirrored sample: both mirror samples were taken low on the screen, which is where the cat *chases*, so it flipped back to facing right in between and the "swing" was the head crossing the box from the mirrored position (~7px) to the normal one (~41px). Unrelated machinery supplying the evidence — the exact fault `card-mechanics` shipped a version of. Measuring `eyeCentreX − skullCentreX` fixes it: both marks sit *inside* the mirrored frame, so their offset is what a reader sees on either facing and does not move when the cat walks. Samples are taken **high** now, above the strip where a pointer provokes a chase.<br><br> **Verified in both directions, because a check that cannot fail is worse than no check.** With the mirror compensation removed, `gaze.mjs` reports **12 of 13** (before the touch fixture below was split out) and the one red is the mirror check — the eye reads −2.72px looking left and −2.96px looking right, moving the wrong way by a quarter of a pixel, with nothing else noticing. With the gaze zeroed at its constants, **7 of 13** — every check that asserts the feature *does* something fails, and the ones asserting an *absence* (reduced motion both ways, the grooming stand-down, the tap expiring) correctly stay green. A third reading came back false and is recorded in the harness header rather than quietly dropped: switching the feature off with an early `return` and rebuilding **with the output discarded** reported 12/13 carrying the *previous* run's failure text verbatim, because the build had not taken and the preview was still serving the older `dist`.<br><br> **And one flake in the harness itself, fixed by making the fixture honest.** The touch check tapped a hard-coded (360, 700) on an 844px viewport — **six pixels** inside `withinNotice`'s boundary, and liable to land on a link. It passed, then failed on a later run with `rotate=none`. The tap point is *computed* now, scanning the strip for a patch of floor that is not a link, and reported through `fixture()` rather than `ok()` so that "the situation never arose" can never again be read as "the feature is broken". At 390px the hero's cover is a full-width `<a>`, so the first version of that scan checked one row, found it entirely link, and correctly refused to pretend otherwise.<br><br>**Gate:** 542 unit tests, `astro check` at 0 errors / 0 warnings / 0 hints, a warning-free build, `gaze.mjs` at 14/14 three runs running, and the snapshot risk measured rather than assumed — `arena` 55/55 and `arena4` 39/39 with the mouse moving during a fight, plus `arena2` 29/29, `arena8` 27/27, `touch-fight` 31/31, `top-state` 18/18, `graze-look` 2/2, `hand` 8/8 and `first-run`. Looked at as well as measured, at deviceScaleFactor 6, because a 9° turn on an 11px head is two pixels at 1× and a screenshot of it proves nothing either way. **Still not playtested by a person** — and this one is a pure charm feature, so that gate matters more here than usual. |
| 2.8 | **The cat becomes a cat (§18), and the drawing stops existing three times.** Asked why the sprite was of such low fidelity, the honest answer was that it had never been drawn, only assembled — and the instruction was to rework it "regardless of the time and cost spending". The silhouette had been authored as **three byte-for-byte copies**, a duplication `CatCard.astro` documented as the *fix* for an earlier divergence; it is now one module, `src/lib/cat-art.ts`, with every class name kept so no animation and no harness selector moved. The first redraw fixed every listed fault and earned the correction that cost the most and taught the most — *"shouldn't it look more like a cat instead of a Pokémon?"* — which named a failure mode this document had not had a word for: **a sequence of local improvements has a direction, and nothing in the loop reports on it.** Four chibi proportions had arrived one improvement at a time, each an improvement on its own terms, every step judged against the step before and none against a cat. §18.6: the two earned marks needed **six** corrections between them, all one mistake — a stroke seated by eye against geometry it no longer matched — resting on two facts, that a round cap is half a stroke-width longer than its path, and that what is drawn is an outline, not a path. Plus a seventh fault no containment check can see: the notch **outgrew its host without moving**, reaching 70% of the ear once §18.3 drew the ears smaller. `scratchpad/marks.mjs` was written before the fix, returned **3/10** naming the causes, and returns 10/10 now; the seatings came from a 730-candidate search over an ink map, not a seventh guess. The collar moved **inside `.cat-head`** — §17's yaw had been sliding the throat out from under a stroke that stayed put — which is the notch's own argument one level up: a mark belongs to the part it is a mark *on*. |
| 2.9 | **The site moves to Next.js, and the game moves with it — not a second copy of it.** Derek supplied two Next.js/PostgreSQL rewrites of the site and asked for them to be incorporated; both deleted the game outright. The chosen path was to migrate *and* re-port it, and the shape of the port is the whole of this row: `SiteCat.astro` (1,707 lines) and `CatCard.astro` (2,816) became `src/lib/site-cat.ts` and `src/lib/cat-card.ts` plus two React components that are markup and a lifecycle call. **The alternative — writing the arena a second time in React — was never on**: eighteen sections of this document specify it and thirty-five harnesses measure it, and two copies means the harnesses keep passing against whichever one they are pointed at while the other drifts unobserved. The DOM contract is untouched on purpose, which is what let **the entire fleet pass against the new build with no change beyond `BASE_URL`** — arena 55/55, arena2 29/29, arena3 34/34, arena4 39/39, arena5 22/22, arena7 26/26, arena8 27/27, battle 25/25, commander 15/15, touch-fight 31/31, the four card harnesses, gaze 14/14, marks 10/10, top-state 18/18. That is the strongest statement available that the game survived: the checks did not move, so the thing they check did not either.<br><br>**`transition:persist` has no counterpart and needs none.** Astro swapped whole documents, so the cat, its treat, its HUD and the card each had to be *marked* to survive a navigation; the App Router keeps the layout mounted and replaces only the page beneath it. The single biggest simplification lands on exactly the two components that were hardest to keep alive. Three places where the Astro code defended against a hazard now have nothing to defend against — `ThemeToggle` re-binding its click per navigation, `A11yControls`' comment that its listeners had to be bound once or they would "stack a new pair on every page load and leave old ones firing against detached header nodes", and `PlaneNav`'s arrow-key listener that would otherwise "fire stale handlers pointing at previous pages' links". All three are effect cleanups now, and the language enforces what the comments used to ask for.<br><br>**The fleet caught a bug both compilers were happy with.** In an Astro `<style>` block, `:global(.cat-tile)` is how you reach a node the component creates at runtime — the compiler strips the wrapper. In a plain CSS file `:global()` is not valid CSS, so the browser **discards the whole rule**. Moving the styles out silently dropped thirteen rules from the card and one from the cat, with `astro check` clean and the build at 0 errors — and `arena` fell from 55/55 to 53/55, reporting a tile with `touch-action: auto` that should refuse panning and a claimed tile whose text contrast had fallen to 1.33 because its wash never applied. Two checks noticed what nothing else could see.<br><br>**Two apparent regressions were chased and neither survived.** `card-mechanics` gave 11/13 and 12/13 on the new build, which looked conclusive until four more runs each: Astro failed it too, and the new build returned 13/13 four times. The check waits eight seconds for the boss to enter a fetch phase while it runs its own stalk-and-pounce cycle — a timing race on both. `hand-look` reported 5/6 against 8/8, and the *total* was the tell: six checks attempted instead of eight means a fixture never dealt and a branch was skipped, not that a check failed.<br><br>**§12's rule finally applied to `arena5`.** Its four trickster checks all rested on having recorded some wind-ups, and when the provocation produced none they reported as four assertion failures — "0 wind-ups", "0 of 0 marked as feints", a bluff rate of "NaN%". Nothing had been measured, so nothing should have been claimed: that is this document's own rule broken inside the fleet that states it. The sample is a fixture now, re-rolled up to three times like any board, and the run log shows the mechanism plainly — attempt 1 "0 phase records, 0 wind-ups", attempt 2 "77 records, 26 wind-ups", every assertion green. **22/22 three runs running**, where it had alternated 22 / 21 / 18 since 2.5. |

---

## 0. What already exists (design constraints, not wishes)

> **File names in this document are historical from 2.9 onward.** The site moved from Astro to
> Next.js and the two big components were split: the state machines are now `src/lib/site-cat.ts`
> and `src/lib/cat-card.ts`, the markup is `src/app/_chrome/SiteCat.tsx` and `CatCard.tsx`, and the
> stylesheets are `src/styles/cat.css` and `cat-card.css`. Nothing about the *mechanics* below
> changed — every id, class and `data-` attribute is what it was, which is why the whole harness
> fleet passes unmodified. Line numbers against `.astro` files are frozen at the moment they were
> written and are kept as provenance, not as directions. See the 2.9 changelog row.

This is **not** a greenfield design. The ambient cat (then `src/components/SiteCat.astro`, 1202
lines) and `src/lib/cat-game.ts` already ship:
...
> §11 records the rule, **amended in 2.1**: the page is silent until a visitor
> presses "cat takes the screen", and from that press the fight is audible without
> asking again. The chip is a mute, not an opt-in.
...
- **Pillar 2 inverts and gets stronger.** The card is `position: fixed` chrome that never adds
  ...
  **This includes flags the card would find convenient.** 2.2.1 wanted the page's paw row to stand
  down while the card is open, and reached for a `cat-card-open` class on `<html>`; `arena.mjs`
  failed it on the first run, and was right to. `<html>` is the page. Where the card needs the page
  to *react* to it, the card sets the state on **its own** element and the page reads it with
  `:has()` — `body:has(.cat-card-root[data-open])` — which is CSS matching, not a mutation, and
  leaves the snapshot untouched. The card owns its attributes and nothing else's.
...
6. **The tiles rendered unstyled.** Tiles are `document.createElement`'d, so they never carry
   ...
   2.2.1 hit this a second time, on the kittens, and the second time is what turns a note into a
   rule. A `.cat-card-kit` with no matching rule is not subtly off: it has no width, no height,
   `position: static` and `fill` at SVG's initial black, so it painted the entire board as a black
   rectangle. **The rule, for anything the card's script creates:**

   - **Every CSS rule targeting a script-created element must be `:global()`.** The scoping
     attribute is applied at build time to markup Astro can see. `createElement` is invisible to it.
   - **Every sprite needs `fill: currentColor` stated.** The cat paths carry no `fill` attribute, so
     without it they take SVG's initial value — black — which happens to resemble `--color-ink` in
     the light theme and is invisible in the dark one. A sprite that only looks right in one theme
     is a sprite nobody checked in the other.

   Both faults were caught by looking at the card, not by a check: the harnesses read `data-state`
   and geometry, and every one of those was correct while the board was a black rectangle.
...
- **Territory on the top edge**, 3px, no numbers. The cat pushes up from the
  ...
  - **2.3 amends the placement, not the rule.** On the card the strip ran full-bleed
    along the panel's very top, where the border-radius clipped its left end and it
    stopped in mid-air with no track behind it: it read as a rendering artefact
    rather than a measure. It is now a 2px inset gauge with a visible track, sitting
    between the header and the board — *directly above the thing it measures*. No
    numbers still, and the direction still carries the fiction. The general lesson
    is that **an edge is not automatically the right place for a gauge**; the right
    place is next to its subject.
...
- **The HUD gives way to the page on a small screen** (2.1). The widget is `position:
  fixed` above the cat and grows *upward*, and it grew sideways too: one control in 1.3,
  two by the end of it, three by 2.0. Nothing in the CSS had ever accounted for a phone's
  aspect ratio, and by 2.0 the labels no longer fit on one line at 390px, so the chips
  wrapped into a stack ~130px tall and stood on the page. Measured at **375×667**:
  `document.elementFromPoint` on the centre of the hero's "See the work" button returned
  the chip. §11 promises links keep working *during* a fight; this was the page not
  working **before** one, which is worse and had never been checked.

  Two rules answer it, both scoped to `max-width: 767px`:

  1. **At rest, one control.** Sound and "which game" are settings for a fight that does
     not exist yet; before a fight there is exactly one thing to do. All three come back
     when the fight starts, because by then the visitor has given the screen to the cat
     and the controls are the way out — and on a phone there is no Esc.
  2. **While the hero's own buttons are in that band, the widget steps aside**: the paw
     row folds away and the chip drops to the very bottom, over the cat's walkway. An
     `IntersectionObserver` whose root is shrunk to the HUD's band decides this, so
     "in the way" means in the way rather than "on screen somewhere".

  **It moves rather than disappears, and that was the second attempt.** Hiding it made the
  hero's button tappable and hid the only way into the game on the page most visitors land
  on — `touch-fight` taps that button on that screen, so the fleet would have gone red for
  a decision made on purpose. A control that vanishes is a worse answer than one that steps
  aside. The first attempt at hiding it did not even work: `pointer-events: none` on the
  container is overridden by `.cat-arena-btn`'s own `pointer-events: auto`, so the chip was
  invisible **and still swallowed the tap** — a fix that photographs correctly and changes
  nothing.
...
**The gate, once it was actually green.** 474 unit tests including the seven hygiene rules,
`astro check` clean, and **430 browser checks across fifteen harnesses green on one build** — plus the
runs that mattered more than the sweep: `arena` twice, `battle` twice and `first-run` three times, each
after its own fix, because every fault in this section presented as a red that came and went. The
fixture lines carry their provenance now (`on deal 2 of 6`, `on deal 4 of 40`), so the next
intermittency arrives as a number rather than as a mystery.
...
> **Superseded twice, and neither move was recorded here until 2.6.2 went looking.** 2.2 moved
> the arena toggle — with the sound and mode chips — **into the card's header**, so `#cat-hud`
> has not held a button since. 2.6.2 then made the paw row itself transient, so the widget this
> section calls "the existing paw widget" is now a single chip that is invisible most of the
> time. What survives is the *reasoning*: the two rejections below still hold, and the corner is
> still the cat's own furniture. Read the placement as history.
...
**2.1 turned it around: sound is on by default and this chip mutes it.** 1.3
argued the opt-in at length, and the argument was about a *page* making noise at a
reader — which this is not. Sound can only happen inside a fight; a fight can only
start by pressing "cat takes the screen"; nobody presses that by accident. Asking
a second time, in a second control, for permission to make the thing you just
started audible is asking a visitor to opt into their own decision, and 2.0's
whole finding was that this game asks too much before anything good happens.

**Consent did not move — it consolidated.** The cat is still silent on every page
until somebody chooses to play, which is the property §11's row actually protects.
What the chip does now is turn sound *off*, which §11 also requires: a shared
desk, a quiet carriage, one tap, without leaving the fight. Removing the control
entirely would have left no way out, so it stays.

The browser's autoplay gate lines up with this rather than against it: audio needs
a user gesture, the arena toggle **is** one, and `primeSfx()` runs inside that
click so the context is unlocked before the first cue. A context created outside a
gesture starts `suspended` — silence that no label would ever admit to — which is
why `commander.mjs` asserts on the context's `state` and on oscillator counts
rather than on the chip's `aria-pressed`.
...
## 15. Commander mode (2.0, retired 2.6) — the kittens fight, you give orders

**Historical.** The half of this that survives is the half that mattered: **the kittens fight**. They
still pick their own work by `pickWork`'s two clocks, the rounds still escalate, losing is still
impossible, and `cat-best-round` is still the one integer stored. What went in 2.6 is the *orders* —
`order()`, `assignOrder`, `Kitten.ordered`, the `'order'` cue and the `teach-order` line — replaced by
§16's swipe as the way a visitor intervenes.

Kept rather than deleted because it is the argument against re-adding ordering. Two clicks was the
thing §16 set out to reduce to one, and `assignOrder` only made ordering *feel* free by working to
hide its cost (idle kitten before busy, so no visible progress was ever spent). `pickWork` was always
doing the harder and more interesting half. Everything below describes the mode as it shipped in 2.0.
...
## 16. The invisible hand (2.5, shipped 2.6) — you disturb the cat, the kittens work

**Status: this is the game.** 2.6 promoted it out from behind `?cat=hand` and **the swipe replaced
§15's click-to-order**, so a visitor now gets one verb where they had two: the kittens pick their own
work and the hand is how you interfere. §15 is kept below as the record of the mode it replaced.

There are still exactly two modes, which is why this is not the "three modes on a portfolio" the flag
was protecting against: `hand` (default) and §3's `manual`, and the chip names the second one.
...
somebody, throw a treat to pull the cat. This asked the opposite question: **what if the player's only
job is interfering with the cat, and the squad gets on with the work by itself?** — and 2.6 answered
it by deleting the first of those two clicks. Treats stay, because throwing one is the site's paid verb
and was never the thing that made the mode two jobs.
...
| `HEADING_TAU_MS` | 220 | The window the cat's heading is smoothed over. **The heading is the cat's own velocity** — the line a player can watch it travel along — and not the direction to its quarry, which is drawn nowhere. See §16.5 fault 7. |
| `HEADING_MIN_SPEED` | 0.2 × `CARD_STALK_SPEED` | Below this the cat is standing and has no axis; `veer` then shoves at full strength in the swipe's own direction, which is what shoving a stationary animal should do. |
| `SWIPE_TRY_FRACTION` | 0.5 | A crossing swipe between this fraction of `SWIPE_MIN_SPEED` and the floor earns the graze tell rather than silence — it was an *attempt*. Below it a pointer is drifting, and answering a drift is noise. |
| `HAND_HINT_EVERY_MS` | 6000 | How often the verb is restated while the player has yet to make contact of any kind. Stops permanently at the first shove or graze. |
| `SHOVE_MIN_DEG` / `SHOVE_MIN` | 20° → `IMPULSE_SPEED · sin 20°` | The along-the-path **dead zone**, stated as the angle the design reasons about with the threshold derived from it. Started at 8.6° and was widened when a browser sweep proved that unreachable — see §16.5 fault 3 and §16.7. Suppresses impulses that travel under 9px against a 28px cat, i.e. nothing that was legible. |
| `GRAZE_MS` | 300 | How long the dead-zone tell shows, and its own throttle. Deliberately **not** `SWIPE_COOLDOWN_MS`: sharing would mean brushing the cat lengthways locked you out of a real shove for half a second, punishing the miss twice and making the dead zone feel like a trap. |
...
**A crossing swipe has exactly two outcomes and both are visible.** `contactFor()` classifies the
impulse as a `shove` or a `graze`, and 2.5.1 exists because the second had no picture: a flick that
**missed** the cat and a flick that **crossed it lengthways** did the same nothing, and a player cannot
learn which half of the gesture to fix from an outcome that never varies. The graze is the informative
one — it means the aim was right and only the angle was wrong. The tell is the tail swept forward over
the back for `GRAZE_MS`, with no accent, no body travel and **no cue**: this is feedback about aim that
a learning player triggers constantly, and a sound on a mistake you are about to repeat is how a
mechanic becomes irritating. `cat-sfx.ts` keeps one cue per visual event; it does not owe every visual
event a cue.

**The pose was chosen by rendering seven candidates at 28px** (`scratchpad/graze-look.mjs`), and two
findings came out of that sheet. The ear does nothing — pinned at -18° on its own it is
indistinguishable from neutral at true size, and the ears already carry `desperate` mood, `sleepy`
stance and the telegraph charge, so a fourth meaning would muddy three existing tells to add none. And
**the best-looking pose was the forbidden one**: rotating the tail negative raises it into a near
vertical stroke, which is precisely the kitten's signature — §0 and 2.3 make "a tail carried straight
up" the one shape the boss never makes, because it is what tells your side from its side at 16px. So the
flick sweeps forward instead, clear of every standing posture (`bored` -32°, `desperate` +20°, `sleepy`
-38°, `groom` -42°) and clear of the squad.
...
**The cat's heading is its own velocity, smoothed over `HEADING_TAU_MS`.** It was `quarry − boss` until
2.5.2, and that is the single change that made the mechanic aimable — see §16.5 fault 7. It is not
`boss.facing` either, which is only ±1 and would make every vertical swipe read as fully "across" a cat
walking straight up the board.
...
3. **An equality test on a floating-point rejection.** `trySwipe`'s no-purchase guard was
   ...
   **And the first zone was too narrow to enter, which only a browser could show.** At 8.6°,
   `scratchpad/hand.mjs` swept a half-circle of flicks at 12° spacing — a spacing that *guarantees* one
   lands within 6° of the estimated heading — and got **14–15 shoves and 0 grazes out of 15**, twice.
   The cause is not the harness and would not spare a finger: a flick takes ~104ms, and the cat's
   heading is the line to a *walking kitten*, so it rotates 6–11° during the gesture (36° when the cat
   is on top of its quarry). A dead zone narrower than its own axis's drift is one nobody can enter.
   Widened to 20°, which is that drift plus room for a person aiming by eye. **It is affordable only
   because the graze now has a tell** — a zone this wide with no feedback would be a mechanic that
   silently ignores a fifth of what you do.
4. **A sentinel that was safe by accident.** `boss.shoveAt` starts at `0` meaning "never shoved", and
   ...
   number.
5. **A "total" type with a silent case, deleted by its own property test.** `contactFor` first returned
   `'shove' | 'graze' | 'none'`, on the reasoning that a zero-magnitude rejection is not really contact
   and the type should say so. The test that walks every angle then found `veer` returning **exactly**
   zero at 0° and 180°, where the arithmetic is axis-aligned and the floating point cancels cleanly — so
   the one function written to remove a silent outcome had a silent one of its own, at precisely the two
   angles a player aiming down the cat's back produces. The caller was accidentally safe, because it
   branched on `!== 'shove'`, which is the worst kind of safe: the next caller to read the type literally
   reintroduces the bug. A zero rejection **is** a graze — you crossed the cat and got no purchase — and
   the case is gone.
6. **A shipped bug with nothing to do with §16, found while reading the handler.** The board's
...
7. **The axis was invisible, and that is why nobody could aim at it.** `SHOVE_MIN`'s dead zone was
   measured unreachable twice — at 8.6° and again at 20° — with a swept half-circle producing 14–15
   shoves and 0 grazes. The zone width was never the problem. The heading was `quarry − boss`: the line
   from the cat to whichever kitten it was chasing, **drawn nowhere on the board**, and rotating 6–11°
   during the ~104ms a flick takes. Asking a player to swipe "across its path" meant asking them to aim
   relative to something they could not see. The heading is now the cat's **own velocity**, smoothed
   over `HEADING_TAU_MS` — the same quantity an eye perceives, and conveniently the smoothed version of
   the line it replaced, which is what damps the drift. Measured effect: **a deliberate graze went from
   ~15 flicks to 1**, on the first flick of the sweep, on every run. The lesson generalises past cats:
   *if a mechanic asks the player to judge a quantity, the quantity has to be on screen.*
...
**`scratchpad/graze-look.mjs` is the fourth: the three outcomes side by side**, at true size and then
magnified, differing only by class. Static on purpose — a graze has no displacement to record, its whole
content is a posture — and it pins the cat with an `!important` stylesheet rule first, because the first
version *claimed* to freeze it and did not: `locator.screenshot()` measures the box and then captures,
and at ~300ms a capture the cat walks its own body width out of the crop. **A comment asserting a freeze
that no code performs**, committed inside the tool built to find exactly that.
...
- **The dead zone now has a tell, and it is deliberately quieter than the shove.** A graze reads at 28px
  and is unmistakable in motion, but it is a silhouette change against a colour change, so a player
  skimming may register only that "nothing happened". That is the intended ranking — a shove is the event
  worth announcing — but it is the first thing to revisit if a playtest says the gesture feels unresponsive.
- **A miss is still silent, and correctly so** — a flick nowhere near the cat gets nothing, because the
  alternative is the game reacting to every pointer movement over the board. But "crossed it too slowly"
  is no longer silent: `SWIPE_TRY_FRACTION` gives a crossing swipe above half the speed floor the same
  graze tell as a lengthways one, because both mean *you touched me and got no purchase*. The two causes
  deliberately share a picture; which remedy to try — faster, or more across — is something a player
  works through anyway, and what they could not recover from was an outcome that never varied.
- **The dead zone is enterable now, and that was the axis rather than the width.** Aiming along the
  cat's visible line of travel grazes on the **first flick**, measured on every run; it took ~15 before
  2.5.2 moved the heading from `quarry − boss` to the cat's own velocity. `hand.mjs` §1 still reports the
  attempt count every run, because a number that moved once can move back. What remains open is a
  question no harness can answer: whether 20° is the *right* width now that it is reachable — wide
  enough to be learnable, narrow enough not to feel unresponsive — and that needs a person.
...
- **The cat teaches the verb itself**, in §8's dialogue table: `teach-swipe` — *"swipe across me. i
  dare you."* — gated to the cold opening exactly as §3's `teach-hold` is for the other mode. 2.5.2 had
  the *caption* teach it, on the reasoning that a flagged mode had no other way to introduce itself;
  2.6 deleted that, because rendered side by side the ribbon and the caption said the same sentence
  thirty pixels apart in two type styles. Same fault as 2.2.1's `treats: 0 / 7 treats`: one thing drawn
  twice.
...
## 17. The gaze (2.7) — the cat looks where you point

**Status: shipped.** Site-wide, not home-only: the cat is `transition:persist` chrome, and one
continuous character that stops noticing you when you click through to `/about/` reads as a bug
rather than as a design.

### 17.1 Purpose

The cat has walked the bottom edge since 0.1 and has never once acknowledged that anybody was
there. Every reward in this document is about the cat's opinion of *you* — the collar, the notch,
the seven rungs from "keeping its distance" to "yours" — and none of it was legible until you had
already found a treat. A head that turns is the cheapest possible *I can see you*, and it is the
first thing on the site that answers the reader before the reader has done anything.

**Experience goal:** a notice, not a toy. The cat should seem to have looked up, not to be
following a cursor as a mechanism. Nothing about it is a control and nothing about it can be won.

### 17.2 Where it came from, and what was actually copied

Asked for from an 11-second recording of an unrelated landing page whose sunflower mascot turns
toward the cursor. The mechanic was **measured rather than imitated** (`scratchpad/measure_ref.py`
— frames via `ffmpeg-static`, because this container's Chromium has no H.264 and Playwright's
bundled ffmpeg is a VP8-only build with three demuxers): the reference's face travels **~17px
horizontally against ~3px vertically**, a 5.7:1 ratio, and the dark region's *area* shrinks at the
extremes. So the thing that reads as "it noticed me" is a **yaw** — a head turning — and not a
nod, and that is what `GAZE_NOD_RATIO` encodes.

One claim is deliberately absent. The first attempt to correlate the effect with the cursor
looked for "near-white, anywhere" and locked onto the page's white headline, reporting the same
coordinate in all eleven frames. That the reference follows the pointer is established by reading
frames, not by a correlation figure, and no figure is quoted for it.

### 17.3 Inputs — all of which already existed

`prey`, the pointer record the cat has kept since 1.1: position, the time of the sighting, a
`hold`, and `here`. Fed by the single `window` `pointermove` and — this is the part that made
"look at where I touch" free — by the touch `pointerdown` path, which logs a tap as
`sight(x, y, 5200)`, one sighting held far longer *because a finger reports once and then there is
no pointer any more*. The gaze reads that record inside the rAF `tick` that already runs.

**No new listener, no new element, no new art.** The whole feature is a reader of existing state.

### 17.4 Outputs

`rotate:` on a new `.cat-gaze` group and `translate:` on `.cat-eye` — the **independent** transform
properties, never `transform:`. Not a stylistic choice: `cat-blink` on the eye, `cat-groom` and
`cat-munch` on the head all animate `transform`, so writing `transform` here would fight three
shipped animations and the cat would stop blinking the moment it looked at you. 2.5 learned this
one level up, when the swat keyframes had to leave `transform` alone because it already carried
the facing flip.

`.cat-gaze` wraps `.cat-head` rather than replacing it, because the head already carries those two
animations about the default origin; giving *that* element a neck origin would silently redraw two
shipped poses.

**And the origin has to be said out loud.** In SVG, `transform-origin` resolves against the *view
box*, so an unset origin would swing the head about (32, 20) — the middle of the sprite, well below
and left of the skull — and the glance would arrive as a shoulder throw. It is `50px 19px`, where
the skull meets the body.

### 17.5 The values, and why they are those values

| Constant | Value | Bound by |
|---|---|---|
| `GAZE_YAW_DEG` | `[PH 9]` | The art. Past ~10° the far ear's tip swings clear of the skull's silhouette and the head reads as *detached* — a two-pixel gap at the rendered 48×30, which is plenty to look broken. |
| `GAZE_EYE_UNITS` | `[PH 1.2]` | The art again. The eye is r=1.15 at (57.5, 11.5) in a skull centred (55, 13) r=7.6; past ~1.3 units its edge reaches the outline, and an eye touching the edge of a head is a mistake in the drawing, not a glance. |
| `GAZE_NOD_RATIO` | `[PH 0.3]` | The measured 5.7:1 reference ratio, and the reason it generalises: a head on a neck turns much further than it tilts. Enough vertical left to tell a pointer in the header from one by the footer. |
| `GAZE_TAU_MS` | `[PH 200]` | `HEADING_TAU_MS = 220` (`hand.ts`) is already justified as the window an eye integrates a direction over. This is the same quantity one step more direct — a literal eye rather than an inferred heading — so slightly quicker. |

Saturation is at **half the viewport in each axis** rather than at a pixel count, so "the far edge
of your screen" is full deflection whatever that screen is, and the gaze does not behave
differently on a phone and a monitor for a reason no reader could name.

### 17.6 Edge cases and failure states

| Failure | Guard |
|---|---|
| **The yaw inverts when the cat faces left.** The flip is `scaleX(-1)` on the `.cat-flip` ancestor, so a rotation inside it is mirrored. | Both horizontal terms negate on `s.dir === -1`. The vertical is untouched — a horizontal mirror does not care which way is up. Asserted on both facings, and the assertion is proved able to fail. |
| A head frozen mid-glance after the pointer leaves | `prey.here` false → ease home |
| A touch sighting held forever | `now - prey.t > prey.hold` → ease home; the existing 5200ms hold is what expires |
| A turned head surviving reduce-motion | The tick bails at the top, so `parkIdle` — the shared exit — clears the properties. What a stood-down cat has is *no transform*, which is checkable, not an identity one |
| Two animations arguing over the head | Muted during `grooming`, `munching`, `dozing`, `stretching`, `perched` and `climb`. The first four pose the head or the sprite; `perched` makes the gaze meaningless (the cat is sitting *on* the cursor, so "look at the pointer" resolves to "look at your own feet"); `climb` has already rotated the whole sprite 90° onto a wall |
| Breaking pillar 2's byte-identical page | Both writes land inside `#site-cat`, **already excluded** from the fleet's page snapshot. Measured rather than assumed: `arena` 55/55 and `arena4` 39/39 with the mouse moving during a fight |

### 17.7 What it does not do

It does not track during a fight, it is not a control, and it earns nothing. The gaze is the one
piece of this document's machinery with no reward attached — which is the point of it.

---

## 18. The redraw (2.8) — the cat becomes a cat

**Status: shipped.** Art, not a mechanic: nothing here changes what the game does. It is in this
document because the drawing turned out to have the same two properties every mechanic has — a
single source of truth, and a claim that can be checked — and it had neither before.

### 18.1 Purpose, and the question that started it

Asked plainly: *"why is the cat of such low fidelity?"* The honest answer was that it had never been
drawn, only assembled — a lozenge body, a circle head, four rounded rectangles for legs, a
uniform-width stroke for a tail, and a dot for an eye. Every mechanic in §§1–17 hangs off that
sprite, and §17 had just given it a head that turns, which is a lot of machinery pointed at a shape
that does not read as an animal.

The instruction that followed was **"rework the cat, regardless of the time and cost spending"**,
which declined an offer to change only the pupil. So the whole silhouette was redrawn.

**Experience goal:** at 48×30 CSS px, against text, a reader should recognise a cat before they
recognise a logo. It stays an ink silhouette — `CatCard.astro` records an earlier attempt at "a
bigger dome and a two-eyed smile" and the verdict that it "made it worse: that is a cartoon, and
this site draws ink silhouettes" — so every gain had to come from the *outline*, which is also the
only part the sprite's `drop-shadow(0 0 2.5px var(--color-ground))` paper halo does not smear.

### 18.2 The drawing was authored three times

The silhouette existed as three byte-for-byte copies: `#site-cat`, `CatCard`'s launcher icon, and
`CatCard`'s boss, whose kittens are cloned from it. `CatCard.astro` said so out loud and treated the
duplication as the **fix** for an earlier divergence — its copy had once lost the legs and the head,
so "the cat was a floating loaf", and the repair was to paste the real drawing back in.

That was the right call at the time and the wrong shape for it. Three copies agreeing today is not a
guarantee; it is a coincidence that had already failed once. `src/lib/cat-art.ts` is now the only
place the drawing exists, and the three components interpolate it.

**Deliberately not a component, and deliberately not `<use>`.** Each copy animates *different parts*
under different class names — `.cat-tail` here, `.cat-card-tail` there, `.cat-card-boss-tail` in the
third — and a `<use>` clone hands you a shadow subtree those rules cannot reach. Exported path
strings keep every existing selector working and still make divergence impossible. No animation and
no harness selector changed.

### 18.3 The correction that cost the most, and was the most useful

The first version fixed every fault listed above and got the verdict: **"shouldn't it look more like
a cat instead of a Pokémon?"**

Correct, and nameable. A big head, a big eye, wide-splayed ears, a deep chest and short legs are
*chibi* proportions, and four of those had crept in one improvement at a time — each one an
improvement on its own terms. A cat in profile has a small head (about a fifth of the body, not a
third), a small eye, ears close together, a shallow chest, and legs about half its standing height.

Head shrunk, eye halved, ears drawn in, chest raised, legs lengthened — **and then the haunch and
shoulder had to be put back**, because flattening the chest to fix the proportions had turned the
torso into a sausage. Each landmark is load-bearing for a different reason, which is why they were
bought back one at a time on the bench rather than tuned by feel.

The general lesson, which is why this paragraph exists: *a sequence of local improvements has a
direction, and nothing in the loop reports on it.* Every step was judged against the step before.
Nobody was comparing against a cat.

### 18.4 Inputs and outputs

| | |
|---|---|
| **Inputs** | Nothing at runtime. The module exports constants; there is no state, no timing and no event. |
| **Outputs** | `CAT_TAIL_D`, `CAT_BODY_D`, `CAT_HEAD_D`, `CAT_EAR_FAR_D`, `CAT_EAR_NEAR_D`, `catLegD(x, lean)`, `CAT_LEGS`, `CAT_EYE`, `CAT_PUPIL`, `CAT_COLLAR_D`, `CAT_NOTCH_D` |
| **Consumers** | `SiteCat.astro` (the ambient cat), `CatCard.astro` (launcher icon, boss, cloned kittens) |
| **Coupled constants** | `GAZE_EYE_UNITS` (§17.5) is derived from `CAT_EYE`/`CAT_PUPIL`, so halving the eye moved it from 0.9 to 0.55 automatically — the point of deriving a number from the drawing rather than from taste |

### 18.5 The hinges still work, because the hinges are keywords

Nothing about the rig changed, and that is not luck: `.cat-leg` hinges at `center top`, `.cat-tail`
at `right bottom`, `.cat-ear` at `center bottom`, each with `transform-box: fill-box`. Keyword
origins resolve against the element's *own* bounding box, so they re-hinge to new geometry for free.
A leg whose bounding-box top edge stops being the hip is the one way to break this, which is why
`catLegD` documents that edge as a constraint rather than as an accident.

The one place this does not hold is §17's gaze, which needs a neck rather than a bounding box:
`transform-box: view-box` with an explicit `transform-origin: 50px 19px`.

### 18.6 The two earned marks, and the failure they share

§7.1's rewards — the collar for patience, the notch for confrontation — are strokes laid over a
silhouette that had just been redrawn underneath them. Between them they needed **six** corrections,
and every one was the same mistake in different clothes: *a stroke seated by eye against geometry it
no longer matched.*

| Attempt | What was wrong |
|---|---|
| Notch, original | Ran to x 62.4 with a 2.2 round cap — half the stroke hung past the ear's outer edge, and the reward read as a flag stuck to the cat's head |
| Notch, 2nd | Pulled inboard, leaving the cap 0.4 units inside the edge, which antialiasing and the paper halo closed up into looking flush anyway |
| Notch, 3rd | Escaped the **inner** edge instead, by the same half cap, into the gap between the ears |
| Collar, original | Sat at x 47.5–49.5 — behind the jaw and inside the body, where nothing showed |
| Collar, 2nd | Hung under the jaw and read as a **tongue**: the head is drawn over it, so the only part left was the inch escaping below the chin |
| Collar, 3rd | Crossed the throat correctly and still put its upper cap in open paper behind the skull's back edge at x 50 |

Two facts account for all six. **A round cap is half a stroke-width longer than its path**, and on a
mark this size that is most of the margin. And **what is drawn is an outline, not a path** — seating
a centreline says nothing about where the ink lands.

There is also a failure no containment check can see: the notch *outgrew its host without moving*.
The §18.3 proportion fix drew the ears smaller and closer, and a mark that had been a nick on the
old ear was **70% of the width** of the new one. That is invisible in a diff and obvious at 8×.

### 18.7 How it is verified

`scratchpad/marks.mjs`, and the rule is the same for both marks: **every point of a mark's stroke
outline — caps included — must fall inside the fill of the shape it is a mark *on*.**

Measured in SVG user space through `getScreenCTM`, so the ear's rotation, the head's, the gaze's and
the sprite's `scaleX(-1)` are all accounted for rather than assumed away; `isPointInFill` is the
browser's own answer to "is this point inside that path". 242 outline points per mark, sampled every
0.02 units along a 1.7-unit notch — well past the resolution of the smallest fault above.

It was written *before* the fix and returned **3/10**, naming the causes; it returns 10/10 now. A
check that has never been red is not a check.

The seatings themselves came out of a search rather than a seventh guess: an ASCII map of which
points are ink at every yaw the head can reach (`neckmap`), then 730 candidate collars tested for
full containment, keeping the longest at the widest stroke that fits.

### 18.8 Edge cases and failure states

| Failure | Guard |
|---|---|
| **A mark that slides off its host when the head turns.** The collar was a sibling of the head, so §17's yaw moved the throat out from under a stroke that stayed put. | The collar is now drawn **inside `.cat-head`, after the skull**. Two orderings, two different jobs: *after the skull* is what makes it visible at all; *inside the head* is what keeps it on the cat. This is the argument the notch already made one level down — a mark belongs to the part it is a mark *on* — and it makes containment a property of the drawing rather than of the pose |
| A mark contained but invisible | Painted after its host, asserted by DOM order rather than by looking |
| A mark and its host drifting apart by a factor | The proportion check: the notch may be at most `[PH 0.55]` of the ear's width. Loose on purpose — it catches a factor, not a tenth |
| The three copies diverging again | There is one copy. `tests/` and the card harnesses cover the rest |
| A pose that only works standing still | `cat-step` swings the legs ±16°, so the bench renders a mid-stride pose as well as a standing one |
| The redraw breaking a shipped animation | Every class name kept, so nothing had a selector to lose. The whole fleet re-run on the new sprite: `arena` 55/55, `arena2` 29/29, `arena3` 34/34, `arena4` 39/39, `arena7` 26/26, `battle` 25/25, `card-check` 21/21, `card-fight` 16/16, `card-mechanics` 13/13, `card-ambient` 5/5, `check-ink` 20/20, `commander` 15/15, `graze-look` 2/2, `hand` 8/8, `hand-look` 8/8, `rail-spy` 10/10, `top-state` 18/18, `touch-fight` 31/31, `gaze` 14/14, `marks` 10/10, plus `first-run` and `cycle` (own reporters) |
| Two harnesses that did **not** come back clean, and neither is the redraw | `arena8` 26/27, where the miss prints `FIXTURE` — no board in 14 deals offered a cat that could pounce, which is the fleet's own way of saying the harness never set itself up. `arena5` returned **22/22 and then 18/22 on back-to-back runs**, matching the 22/21/18 already recorded against it; its four failures are `0 wind-ups` and `0 of 0 marked as feints (NaN%)`, which is an assertion standing on a sample that was never collected — §12's fault, in the harness rather than in the game, and logged to be fixed there |

### 18.9 What it does not do

It adds no interior detail, no second eye, no colour beyond the two earned marks, and no new
element. A silhouette that needs an interior line to read has failed at the thing a silhouette is
for.
