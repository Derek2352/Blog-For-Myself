...
**Version** 2.5.2 · **the card is the only game — the fight leaves the page, and the page is left alone.** Five versions of
...
- **The mode has no chip**, by design for a prototype — three modes on a portfolio is too much game, and
  the flag stays. Within the mode the verb is now discoverable: the caption naming it is restated every
  `HAND_HINT_EVERY_MS` until the player has made contact of either kind, then never again, because
  `arenaCaption` overwrites it on the first reclaim and that can happen before anyone has swiped once.
