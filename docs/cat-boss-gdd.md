...
**Version** 2.5.1 · **the card is the only game — the fight leaves the page, and the page is left alone.** Five versions of
...
- **A miss is still silent, and correctly so.** A flick nowhere near the cat gets nothing, because the
  alternative is the game reacting to every pointer movement over the board. So the player still cannot
  distinguish "missed" from "did not swipe fast enough" — `SWIPE_MIN_SPEED` has no tell at all, which is
  the same class of fault one level down and was left alone rather than fixed blind.
- **The dead zone is hard to enter on purpose, and this is the open design question.** Even at 20°, a
  graze takes on the order of fifteen flicks to produce deliberately — `hand.mjs` §1 reports the attempt
  count on every run precisely so the number stays visible. The reason is structural rather than a
  matter of degree: **the axis the player is asked to aim along is invisible and moving.** It is the
  line from the cat to a walking kitten, which is drawn nowhere on the board, and it rotates during the
  gesture itself. So §16's headline promise — *across shoves, along does not, and the angle you choose
  is the whole skill* — is only half true in play: the `|sin θ|` ramp genuinely modulates strength, but
  the categorical "along does nothing" is not something a player can reliably aim for.

  Three ways out, none taken yet because each is a design decision rather than a fix: widen the zone
  further (costs responsiveness, and the drift eats most of the gain); **define the heading as the
  cat's own velocity rather than the direction to its quarry**, which is at least a line the player can
  *see* the cat travelling along — and which is what `trySwipe`'s comment already claims it does; or
  accept that the ramp is the mechanic and drop the categorical claim from the design's language. The
  second is the most promising and the most honest to the gesture, and it is the first thing to try if
  a playtest says the angle does not feel like it matters.
